...
def main(argv):
    ...
    funct_dict = {"4-2": authen_simple.evaluate_prediction_vs_full_law,
    ...
    for system_folder in system_folders:
        ...
        for dataset_file in dataset_files:
            ...
            # 如果是4-2，从generated_10.json中加载authen计算数据
            if datafile_name == "4-2":
                print("Using law_articles from generated_10.json for 4-2 authen score")
                with open("/home/yxma/hzx/LeLLM/ckpt/predictions/zero_shot/4-1-test.json", "r", encoding="utf-8") as f:
                    gen_data = json.load(f)

                # 构造一个与 data_dict 结构一致的list，每个元素是{"prediction": ...}
                law_articles_list = [{"prediction": item["law_articles"]} for item in gen_data]
                auth_result = authen_simple.evaluate_prediction_vs_full_law(law_articles_list)

                print(f"Score of 4-2 (authen): {auth_result}")
                results["task"].append("4-2")
                results["model_name"].append(system_folder)
                results["score"].append(auth_result["score"])
                abstention_rate = auth_result["abstention_rate"] if "abstention_rate" in auth_result else 0
