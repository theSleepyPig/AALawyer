#+similar cases
# nohup python /home/yxma/hzx/LeLLM/aiEval/generate_response_json_inter_v1.py \
#     > generate_response_json_inter_run2_v2.log 2>&1 &
import csv
import json
import re
import os
...
output_json = "results/generated_200_inter_v2023_run1.json"  # 改为 json 输出
...
print("正在检查并修正模型内部张量设备...")
for name, module in model.named_modules():
    # 定位到 rotary embedding 模块
    if "rotary_emb" in name and hasattr(module, "inv_freq"):
        # 检查它的 inv_freq 属性是否在CPU上
        if module.inv_freq.device.type == 'cpu':
            print(f"发现 {name}.inv_freq 在 CPU, 正在强制移动到 {model.device}...")
            # 将其强制移动到和模型主体一样的设备上
            module.inv_freq = module.inv_freq.to(model.device)
