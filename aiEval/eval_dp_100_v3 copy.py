...
2. 准确性：法条引用的准确性 = 法条编号匹配的准确度 × 对应法条内容描述的真实性。该指标只与两个因素有关，一是是否和参考法条编号一致，二是法条内容是否完整、真实、可靠。与分析内容质量无关。（因为数据标注原因，【参考法条编号】以外的法条也可能是正确法条，可自行判断一下，但【参考法条编号】是最重要的，权重最大）
3. 丰富度：分析是否结构清晰、内容完整，是否结合了案例具体事实与法条条文进行深入解释，是否包含补充信息或合理推理。是否能为法律从业者提供丰富的参考。
4. 可解释性：生成分析内容的透明度。即，分析内容是否含有可靠材料支撑，如真实法条和相关判决案件等。法律从业者能否感到该分析很可靠。
...
evaluate_to_csv("results/generated_200_m20.json", "results/Copy/eval_scores_200_4dim.csv")
